...
class WanVideoDiT(torch.nn.Module):
    ...
    def post_dit(
        ...
        *,
        ...
        velocity = unpatchify_future_tubes(
            full_velocity,
            future_x.shape[2],
            pre_state["meta"]["height"],
            pre_state["meta"]["width"],
        )
        if not return_raw_prediction:
            return velocity
        if self.prediction_type != "x0":
            raise ValueError(
                "`return_raw_prediction` is only supported for x-prediction"
            )
        pred_x0 = unpatchify_future_tubes(
            head_output,
            future_x.shape[2],
            pre_state["meta"]["height"],
            pre_state["meta"]["width"],
        )
        return velocity, pred_x0
