...
def create_fastwam_joint_jit_pixel(
    model_id: str,
    tokenizer_model_id: str,
    video_dit_config,
    tokenizer_max_len: int = 512,
    ...
    from .models.wan22.jit_pixel_fastwam_joint import FastWAMJointJiTPixel

    def as_dict(value, name: str, *, required: bool = False):
        if isinstance(value, DictConfig):
            value = OmegaConf.to_container(value, resolve=True)
        if value is None:
            if required:
                raise ValueError(f"`{name}` is required for JiT pixel FastWAM-Joint")
            return {}
        if not isinstance(value, dict):
            raise ValueError(f"`{name}` must resolve to a dict, got {type(value)}")
        return value

    video_dit_config = as_dict(video_dit_config, "video_dit_config", required=True)
    action_dit_config = as_dict(action_dit_config, "action_dit_config")
    video_scheduler = as_dict(video_scheduler, "video_scheduler")
    action_scheduler = as_dict(action_scheduler, "action_scheduler", required=True)
    loss = as_dict(loss, "loss")
    jit = as_dict(jit, "jit", required=True)

    required_action_scheduler_keys = {"train_shift", "infer_shift", "num_train_timesteps"}
    missing = required_action_scheduler_keys - set(action_scheduler)
    if missing:
        raise ValueError(f"`action_scheduler` missing required keys: {sorted(missing)}")
    required_jit_keys = {"p_mean", "p_std", "noise_scale", "t_eps"}
    missing = required_jit_keys - set(jit)
    if missing:
        raise ValueError(f"`jit` missing required keys: {sorted(missing)}")

    return FastWAMJointJiTPixel.from_wan22_pretrained(
        device=device,
        torch_dtype=model_dtype,
        model_id=model_id,
        tokenizer_model_id=tokenizer_model_id,
