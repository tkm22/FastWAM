...
RUN_ID="${1:-$(date +%Y-%m-%d_%H%M%S)_vr_lpips_Aall}"
...
echo "features=fresh 10ep A=all222929 timestep_sampling=logit_normal vr=true lpips=true"
...
bash scripts/eval_asym_vr_lpips_raw.sh "${RUN_ID}" "${FINAL_STEP}" "${PROJECTION_ARTIFACT}"
